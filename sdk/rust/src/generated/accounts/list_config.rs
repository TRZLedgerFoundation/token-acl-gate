//! This code was AUTOGENERATED using the codoma library.
//! Please DO NOT EDIT THIS FILE, instead use visitors
//! to add features, then rerun codoma to update it.
//!
//! <https://github.com/trzledgerfoundation-idl/codoma>
//!

use borsh::BorshDeserialize;
use borsh::BorshSerialize;
use trezoa_pubkey::Pubkey;

#[derive(BorshSerialize, BorshDeserialize, Clone, Debug, Eq, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct ListConfig {
    pub discriminator: u8,
    #[cfg_attr(
        feature = "serde",
        serde(with = "serde_with::As::<serde_with::DisplayFromStr>")
    )]
    pub authority: Pubkey,
    #[cfg_attr(
        feature = "serde",
        serde(with = "serde_with::As::<serde_with::DisplayFromStr>")
    )]
    pub seed: Pubkey,
    pub mode: u8,
    pub wallets_count: u64,
}

pub const LIST_CONFIG_DISCRIMINATOR: u8 = 1;

impl ListConfig {
    pub const LEN: usize = 74;

    /// Prefix values used to generate a PDA for this account.
    ///
    /// Values are positional and appear in the following order:
    ///
    ///   0. `ListConfig::PREFIX`
    ///   1. authority (`Pubkey`)
    ///   2. seed (`Pubkey`)
    pub const PREFIX: &'static [u8] = "list_config".as_bytes();

    pub fn create_pda(
        authority: Pubkey,
        seed: Pubkey,
        bump: u8,
    ) -> Result<trezoa_pubkey::Pubkey, trezoa_pubkey::PubkeyError> {
        trezoa_pubkey::Pubkey::create_program_address(
            &[
                "list_config".as_bytes(),
                authority.as_ref(),
                seed.as_ref(),
                &[bump],
            ],
            &crate::ABL_ID,
        )
    }

    pub fn find_pda(authority: &Pubkey, seed: &Pubkey) -> (trezoa_pubkey::Pubkey, u8) {
        trezoa_pubkey::Pubkey::find_program_address(
            &["list_config".as_bytes(), authority.as_ref(), seed.as_ref()],
            &crate::ABL_ID,
        )
    }

    #[inline(always)]
    pub fn from_bytes(data: &[u8]) -> Result<Self, std::io::Error> {
        let mut data = data;
        Self::deserialize(&mut data)
    }
}

impl<'a> TryFrom<&trezoa_account_info::AccountInfo<'a>> for ListConfig {
    type Error = std::io::Error;

    fn try_from(account_info: &trezoa_account_info::AccountInfo<'a>) -> Result<Self, Self::Error> {
        let mut data: &[u8] = &(*account_info.data).borrow();
        Self::deserialize(&mut data)
    }
}

#[cfg(feature = "fetch")]
pub fn fetch_list_config(
    rpc: &trezoa_client::rpc_client::RpcClient,
    address: &trezoa_pubkey::Pubkey,
) -> Result<crate::shared::DecodedAccount<ListConfig>, std::io::Error> {
    let accounts = fetch_all_list_config(rpc, &[*address])?;
    Ok(accounts[0].clone())
}

#[cfg(feature = "fetch")]
pub fn fetch_all_list_config(
    rpc: &trezoa_client::rpc_client::RpcClient,
    addresses: &[trezoa_pubkey::Pubkey],
) -> Result<Vec<crate::shared::DecodedAccount<ListConfig>>, std::io::Error> {
    let accounts = rpc
        .get_multiple_accounts(addresses)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
    let mut decoded_accounts: Vec<crate::shared::DecodedAccount<ListConfig>> = Vec::new();
    for i in 0..addresses.len() {
        let address = addresses[i];
        let account = accounts[i].as_ref().ok_or(std::io::Error::new(
            std::io::ErrorKind::Other,
            format!("Account not found: {}", address),
        ))?;
        let data = ListConfig::from_bytes(&account.data)?;
        decoded_accounts.push(crate::shared::DecodedAccount {
            address,
            account: account.clone(),
            data,
        });
    }
    Ok(decoded_accounts)
}

#[cfg(feature = "fetch")]
pub fn fetch_maybe_list_config(
    rpc: &trezoa_client::rpc_client::RpcClient,
    address: &trezoa_pubkey::Pubkey,
) -> Result<crate::shared::MaybeAccount<ListConfig>, std::io::Error> {
    let accounts = fetch_all_maybe_list_config(rpc, &[*address])?;
    Ok(accounts[0].clone())
}

#[cfg(feature = "fetch")]
pub fn fetch_all_maybe_list_config(
    rpc: &trezoa_client::rpc_client::RpcClient,
    addresses: &[trezoa_pubkey::Pubkey],
) -> Result<Vec<crate::shared::MaybeAccount<ListConfig>>, std::io::Error> {
    let accounts = rpc
        .get_multiple_accounts(addresses)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
    let mut decoded_accounts: Vec<crate::shared::MaybeAccount<ListConfig>> = Vec::new();
    for i in 0..addresses.len() {
        let address = addresses[i];
        if let Some(account) = accounts[i].as_ref() {
            let data = ListConfig::from_bytes(&account.data)?;
            decoded_accounts.push(crate::shared::MaybeAccount::Exists(
                crate::shared::DecodedAccount {
                    address,
                    account: account.clone(),
                    data,
                },
            ));
        } else {
            decoded_accounts.push(crate::shared::MaybeAccount::NotFound(address));
        }
    }
    Ok(decoded_accounts)
}

#[cfg(feature = "trezoaanchor")]
impl trezoaanchor_lang::AccountDeserialize for ListConfig {
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> trezoaanchor_lang::Result<Self> {
        Ok(Self::deserialize(buf)?)
    }
}

#[cfg(feature = "trezoaanchor")]
impl trezoaanchor_lang::AccountSerialize for ListConfig {}

#[cfg(feature = "trezoaanchor")]
impl trezoaanchor_lang::Owner for ListConfig {
    fn owner() -> Pubkey {
        crate::ABL_ID
    }
}

#[cfg(feature = "trezoaanchor-idl-build")]
impl trezoaanchor_lang::IdlBuild for ListConfig {}

#[cfg(feature = "trezoaanchor-idl-build")]
impl trezoaanchor_lang::Discriminator for ListConfig {
    const DISCRIMINATOR: &[u8] = &[0; 8];
}
